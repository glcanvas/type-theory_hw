...
let solve_equations equations  =
    let can_update = ref true in
        ...
        while !can_update do
            can_update := false;
            (*term = var -> var = term*)
            can_update := List.exists (fun item ->if (not (is_var (fst item))) && (is_var (snd item)) then true else false) equations;
            let first = List.map (fun item -> if (not (is_var (fst item))) && (is_var (snd item)) then ((snd item), (fst item)) else item) equations in
                ...
                can_update := List.exists (fun item ->if (not (is_var (fst item))) && (not (is_var (snd item))) then true else false) equations;
                let second = List.map() in 
        done;;
